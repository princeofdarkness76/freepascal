...
declare -r APPENGINE_API_JAR="${GCD_DIR}/.appengine/lib/impl/appengine-api.jar"
...
if [ ! -e "${APPENGINE_API_JAR}" ]; then
    echo "${APPENGINE_API_JAR} not found"
    exit 1
fi
...
${JAVA:="java"} -cp "${DATASTORE_JAR}:${APPENGINE_API_JAR}:${APPENGINE_TOOLS_JAR}:${APPENGINE_API_STUBS_JAR}" \
